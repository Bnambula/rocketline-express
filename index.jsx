...
// ─── MTN / AIRTEL PREFIX DETECTION ───────────────────────────────────
const MTN_PREFIXES   = ["077","078","076","039","078","079"]; // MTN Uganda
const AIRTEL_PREFIXES= ["070","075","074","071","020"]; // Airtel Uganda

const detectNetwork=(phone)=>{
  const digits=phone.replace(/[^0-9]/g,"");
  // normalise: strip leading 256 country code
  const local = digits.startsWith("256") ? "0"+digits.slice(3) : digits;
  const pre3 = local.slice(0,3);
  if(MTN_PREFIXES.includes(pre3))    return "mtn";
  if(AIRTEL_PREFIXES.includes(pre3)) return "airtel";
  return null;
};
          ...
          <div>
            ...
            <SeatMap capacity={14} bookedSeats={bookedS} reservedSeats={reservedS} selected={seats}
              onSelect={n=>setSeats(prev=>{
                ...
                if(prev.length>=6){return prev;} // hard limit
                return [...prev,n];
              })}/>
          </div>
...
// ═══════════════════════════════════════════════════════════════════════
const UG_CITIES=[
  {name:"Kampala",x:48,y:52,main:true},
  {name:"Gulu",x:47,y:16,route:true},
  {name:"Mbale",x:68,y:44,route:true},
  {name:"Jinja",x:57,y:53,route:true},
  {name:"Mbarara",x:37,y:68,route:true},
  {name:"Fort Portal",x:22,y:44,route:true},
  {name:"Arua",x:22,y:16,route:true},
];

const UgandaPresenceMap=({setPage})=>{
  const [hover,setHover]=useState(null);
  return(
    <section aria-label="Our Presence in Uganda" style={{padding:"clamp(56px,7vw,88px) clamp(16px,4vw,40px)",background:"#fff"}}>
      <div style={{maxWidth:1280,margin:"0 auto"}}>
        <div style={{textAlign:"center",marginBottom:48}}>
          ...
          <div style={{width:60,height:3,background:C.amber,borderRadius:2,margin:"0 auto"}}/>
          <p style={{fontSize:14,color:C.textSecondary,marginTop:16,maxWidth:500,margin:"16px auto 0"}}>Connecting communities across Uganda — from Arua in the northwest to Mbale in the east</p>
        </div>
        <div style={{display:"flex",gap:40,alignItems:"flex-start",flexWrap:"wrap",justifyContent:"center"}}>
          {/* Uganda SVG map */}
          <div style={{position:"relative",width:360,minWidth:280,flexShrink:0}}>
            <svg viewBox="0 0 100 130" style={{width:"100%",filter:"drop-shadow(0 8px 24px rgba(11,30,75,0.12))"}}>
              {/* Uganda outline — simplified polygon */}
              <path d="M28,2 L35,2 L50,5 L65,3 L78,10 L82,22 L85,35 L82,50 L85,65 L80,80 L72,92 L60,102 L50,108 L42,105 L30,95 L20,80 L15,65 L12,50 L14,35 L18,22 L22,12 Z"
                fill="#DBEAFE" stroke="#93C5FD" strokeWidth="1.2"/>
              {/* Lake Victoria (bottom right) */}
              <ellipse cx="62" cy="95" rx="18" ry="12" fill="#BFDBFE" stroke="#60A5FA" strokeWidth="0.8" opacity=".7"/>
              <text x="54" y="97" fontSize="3.5" fill="#3B82F6" fontFamily="Inter,sans-serif" opacity=".8">Lake</text>
              <text x="52" y="101" fontSize="3.5" fill="#3B82F6" fontFamily="Inter,sans-serif" opacity=".8">Victoria</text>
              {/* Lake Albert (west) */}
              <ellipse cx="14" cy="40" rx="5" ry="10" fill="#BFDBFE" stroke="#60A5FA" strokeWidth="0.7" opacity=".6"/>

              {/* Route lines from Kampala */}
              {UG_CITIES.filter(c=>c.route).map(c=>(
                <line key={c.name} x1="48" y1="52" x2={c.x} y2={c.y}
                  stroke={C.blue} strokeWidth=".7" strokeDasharray="2,2" opacity=".5"/>
              ))}
              ...
              {UG_CITIES.map(c=>(
                <g key={c.name} style={{cursor:"pointer"}}
                  ...
                  {/* Pin drop shape */}
                  <ellipse cx={c.x} cy={c.y+0.5} rx="1.5" ry=".6" fill="rgba(0,0,0,0.15)"/>
                  <path d={`M${c.x},${c.y-7} a4,4 0 1,1 0,0.01 L${c.x},${c.y}`}
                    fill={c.main?"#F59E0B":hover===c.name?"#1D4ED8":"#2563EB"}
                    stroke="#fff" strokeWidth=".7"
                    style={{transform:`scale(${hover===c.name?1.2:1})`,transformOrigin:`${c.x}px ${c.y}px`,transition:"all .2s"}}/>
                  <circle cx={c.x} cy={c.y-5} r="1.4" fill="#fff" opacity=".8"/>
                  {/* Label */}
                  <text x={c.x+(c.x>50?2:-2)} y={c.y-8.5} fontSize="3.2"
                    textAnchor={c.x>50?"start":"end"}
                    fill={c.main?"#92400E":"#1E3A5F"}
            ...
            <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:12}}>
              {UG_CITIES.map(c=>(
                <div key={c.name}
                  ...
                  onMouseEnter={()=>setHover(c.name)}
                  onMouseLeave={()=>setHover(null)}
                  style={{padding:"14px 16px",borderRadius:14,border:`1.5px solid ${hover===c.name?C.blue:C.navyBorder}`,background:hover===c.name?C.blueBg:"#fff",cursor:"pointer",transition:"all .2s",display:"flex",alignItems:"center",gap:12}}>
                  <div style={{width:36,height:36,borderRadius:"50%",background:c.main?C.amber+"22":C.blue+"18",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                    <svg width="18" height="22" viewBox="0 0 24 30"><path d="M12 2C7.13 2 3 6.13 3 11c0 7 9 17 9 17s9-10 9-17c0-4.87-4.13-9-9-9zm0 12a3 3 0 100-6 3 3 0 000 6z" fill={c.main?C.amber:C.blue}/></svg>
                  </div>
                  <div>
                    <div className="ral" style={{fontWeight:800,fontSize:14,color:C.navy}}>{c.name}</div>
                    <div style={{fontSize:11,color:C.textMuted}}>{c.main?"HQ — Kampala":"Active route"}</div>
                  </div>
                </div>
              ))}
            </div>
            <div style={{marginTop:20,padding:"14px 16px",background:C.blueBg,borderRadius:14,border:`1px solid ${C.blue}33`}}>
              <div style={{fontSize:13,color:C.navy,fontWeight:600,marginBottom:4}}>📍 Headquartered in Nakasero, Kampala</div>
              <div style={{fontSize:12,color:C.textSecondary,lineHeight:1.7}}>Serving 6 major cities and growing. All routes depart from the Raylane Express terminal in Kampala.</div>
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
          <div style={{background:C.darkCard,borderRadius:20,padding:28,width:"100%",maxWidth:440,border:`1px solid ${C.darkBorder}`}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:14}}>
              ...
              <Input label="Logo URL or Emoji" value={form.logo} onChange={e=>setForm({...form,logo:e.target.value})} placeholder="https://... or 🏛️"/>
        ...
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
          <div style={{background:C.darkCard,borderRadius:20,padding:28,width:"100%",maxWidth:480,border:`1px solid ${C.darkBorder}`}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:14}}>
              ...
              <Input label="Image URL" value={form.img} onChange={e=>setForm({...form,img:e.target.value})} placeholder="https://... (direct image link)"/>
      ...
      {showModal&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
          <div style={{background:C.darkCard,borderRadius:20,padding:28,width:"100%",maxWidth:460,border:`1px solid ${C.darkBorder}`}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:14}}>
              <Input label="Image URL" value={form.url} onChange={e=>setForm({...form,url:e.target.value})} placeholder="https://... (direct image link)"/>
