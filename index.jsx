...
const MTNLogo=({size=36})=>(
  <svg width={size} height={size} viewBox="0 0 100 100" style={{borderRadius:12}}>
...
@keyframes heroDrift{0%{transform:translate(0,0) rotate(0deg);}33%{transform:translate(20px,-12px) rotate(2deg);}66%{transform:translate(-8px,-22px) rotate(-1deg);}100%{transform:translate(0,0) rotate(0deg);}}
@keyframes bgPan{0%{transform:scale(1.08) translateX(0);}100%{transform:scale(1.08) translateX(-3%);}}
@keyframes bgSlide{0%{opacity:0;transform:scale(1.06);}10%{opacity:1;}85%{opacity:1;transform:scale(1);}100%{opacity:0;transform:scale(.98);}}
@keyframes particleDrift{0%{transform:translateY(0) translateX(0);opacity:0;}20%{opacity:.6;}80%{opacity:.4;}100%{transform:translateY(-120px) translateX(30px);opacity:0;}}
@keyframes textReveal{from{opacity:0;transform:translateY(30px) skewY(2deg);}to{opacity:1;transform:translateY(0) skewY(0);}}
@keyframes amberGlow{0%,100%{text-shadow:0 0 20px ${C.amber}44;}50%{text-shadow:0 0 40px ${C.amber}88,0 0 80px ${C.amber}22;}}
...
.amber-glow{animation:amberGlow 3s ease-in-out infinite;}
    ...
    <>
      {images.map((img,i)=>(
        <div key={img.id} style={{
          ...
          animation:i===idx?"bgSlide 5s ease forwards":"none",
          opacity:i===idx?1:0,
          transition:"opacity .8s ease",
          zIndex:0
        }}/>
      ))}
      <div style={{position:"absolute",inset:0,background:`linear-gradient(to right,${C.navy}ee 40%,${C.navy}88 70%,${C.navy}44 100%)`,zIndex:1}}/>
    </>
  );
};
  ...
  <div style={{position:"absolute",inset:0,overflow:"hidden",pointerEvents:"none",zIndex:1}}>
    {[...Array(14)].map((_,i)=>(
      <div key={i} style={{
        ...
        height:i%3===0?3:2,
        borderRadius:"50%",
        background:i%4===0?C.amber:i%4===1?C.blue:C.white,
        opacity:.25+i*.02,
        animation:`particleDrift ${6+i*1.2}s ease-in-out ${i*.4}s infinite`
      }}/>
    ))}
  </div>
);
...
const HomePage=({setPage,setPreselectedTrip,store,setLoginRole=()=>{}})=>{
  ...
  return(
    <div style={{minHeight:"100vh",paddingTop:64}}>
      {/* ── CINEMATIC HERO ── */}
      <div style={{position:"relative",minHeight:600,display:"flex",alignItems:"center",overflow:"hidden",background:C.navy}}>
        {/* Uploaded background images slideshow */}
        <HeroBackground images={store.heroImages}/>

        {/* Animated gradient mesh (always shown, behind images) */}
        <div style={{position:"absolute",inset:0,zIndex:store.heroImages.length?2:1}}>
          <div style={{position:"absolute",inset:0,background:`radial-gradient(ellipse 80% 60% at 20% 50%,${C.amber}0D,transparent 60%),radial-gradient(ellipse 60% 80% at 80% 30%,${C.blue}0A,transparent 55%),radial-gradient(ellipse 40% 40% at 50% 80%,${C.navyLight},transparent 50%)`}}/>
          {/* Animated mesh lines */}
          <svg style={{position:"absolute",inset:0,width:"100%",height:"100%",opacity:.08}} preserveAspectRatio="none">
            <defs><pattern id="grid" width="60" height="60" patternUnits="userSpaceOnUse"><path d="M 60 0 L 0 0 0 60" fill="none" stroke={C.amber} strokeWidth=".8"/></pattern></defs>
        ...
        {/* Hero floating van illustration */}
        <div className="hero-float" style={{position:"absolute",right:"5%",top:"50%",transform:"translateY(-50%)",zIndex:3,opacity:.18,pointerEvents:"none"}}>
          <svg width="340" height="200" viewBox="0 0 340 200">
            <rect x="20" y="40" width="300" height="130" rx="30" fill={C.amber} opacity=".8"/>
            <rect x="30" y="50" width="120" height="80" rx="12" fill={C.navy} opacity=".9"/>
            <rect x="34" y="54" width="112" height="72" rx="9" fill={C.navyLight} opacity=".5"/>
            <rect x="170" y="60" width="130" height="60" rx="8" fill={C.navyMid} opacity=".6"/>
            <circle cx="80" cy="180" r="22" fill={C.navyBorder}/>
            <circle cx="80" cy="180" r="12" fill={C.navy}/>
            <circle cx="260" cy="180" r="22" fill={C.navyBorder}/>
            <circle cx="260" cy="180" r="12" fill={C.navy}/>
            <rect x="290" y="75" width="40" height="25" rx="6" fill={C.amber} opacity=".9"/>
            <rect x="10" y="90" width="18" height="40" rx="5" fill={C.amberLight} opacity=".7"/>
          </svg>
        </div>

        {/* Hero content */}
        <div style={{maxWidth:1200,margin:"0 auto",position:"relative",width:"100%",padding:"90px 20px 70px",zIndex:4}}>
          <div style={{maxWidth:620}}>
            <div style={{display:"inline-flex",alignItems:"center",gap:8,background:C.green+"22",border:`1px solid ${C.green}44`,borderRadius:20,padding:"5px 14px",marginBottom:22,fontSize:12,color:C.green,fontWeight:700,animation:"fadeUp .3s ease"}}>
              <span className="live-dot"/> Live · {store.trips.filter(t=>store.seatsAvailable(t)>0).length} trips available today
            </div>
            <h1 className="ral amber-glow" style={{fontSize:"clamp(38px,5.5vw,68px)",fontWeight:900,lineHeight:1.04,letterSpacing:"-2px",marginBottom:18,animation:"textReveal .6s ease"}}>
              Travel Across<br/><span style={{color:C.amber}}>Uganda</span><br/>With Confidence
            </h1>
            <p style={{fontSize:15,color:C.textSecondary,maxWidth:480,lineHeight:1.85,marginBottom:32,animation:"fadeUp .7s ease"}}>
              Fast, reliable transport from Kampala to every corner of Uganda. Book your seat in under 2 minutes.
            </p>
            <div style={{display:"flex",gap:10,flexWrap:"wrap",marginBottom:28,animation:"fadeUp .8s ease"}}>
              <Btn onClick={()=>setPage("book")} style={{fontSize:14,padding:"14px 30px",boxShadow:`0 8px 24px ${C.amber}44`}}>🎫 Book Your Ride</Btn>
              <Btn onClick={()=>setPage("plan")} variant="navy" style={{fontSize:14,padding:"14px 24px",border:`1px solid ${C.navyBorder}`,backdropFilter:"blur(8px)"}}>📅 Plan Journey</Btn>
              <Btn onClick={()=>setPage("schedule")} variant="navy" style={{fontSize:14,padding:"14px 24px",border:`1px solid ${C.navyBorder}`,backdropFilter:"blur(8px)"}}>📋 Schedule</Btn>
            </div>
            <div style={{display:"flex",gap:20,fontSize:12,color:C.textMuted,flexWrap:"wrap",animation:"fadeUp .9s ease"}}>
              <span style={{display:"flex",alignItems:"center",gap:6}}>
                <MTNLogo size={22}/> MTN MoMo
              </span>
              <span style={{display:"flex",alignItems:"center",gap:6}}>
                <AirtelLogo size={22}/> Airtel Money
              </span>
        ...
        {/* Bottom fade */}
        <div style={{position:"absolute",bottom:0,left:0,right:0,height:80,background:`linear-gradient(to bottom,transparent,${C.navy})`,zIndex:5}}/>
      </div>
      ...
      <div style={{background:C.navyMid,borderBottom:`1px solid ${C.navyBorder}`,padding:"11px 20px"}}>
        <div style={{maxWidth:1200,margin:"0 auto",display:"flex",alignItems:"center",justifyContent:"space-between",flexWrap:"wrap",gap:8}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <span className="live-dot"/><span className="ral" style={{fontWeight:800,color:C.amber,fontSize:14}}>Departing Soon</span>
            <span style={{color:C.textMuted,fontSize:13}}>— {store.trips.length} trips scheduled today</span>
          </div>
      ...
      {/* URGENT TRIP CARDS — max 2 */}
      <div style={{maxWidth:1200,margin:"0 auto",padding:"28px 20px"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16,flexWrap:"wrap",gap:8}}>
          <h2 className="ral" style={{fontSize:20,fontWeight:800}}>Next Departures</h2>
          <button onClick={()=>setPage("schedule")} style={{background:"none",border:`1px solid ${C.navyBorder}`,color:C.textMuted,borderRadius:8,padding:"6px 14px",fontSize:12,cursor:"pointer",fontFamily:"'Nunito',sans-serif"}}>
            View all {store.trips.length} trips →
          </button>
        </div>
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(300px,1fr))",gap:18}}>
          {urgentTrips.length>0
            ...
            : <div style={{color:C.textMuted,fontSize:13,padding:"20px 0"}}>No upcoming trips at this time. <button onClick={()=>setPage("schedule")} style={{color:C.amber,background:"none",border:"none",cursor:"pointer",fontFamily:"'Nunito',sans-serif"}}>View full schedule →</button></div>
          }
        </div>
      </div>
      ...
      <div style={{background:C.navyMid,borderTop:`1px solid ${C.navyBorder}`,borderBottom:`1px solid ${C.navyBorder}`}}>
        <div style={{maxWidth:1200,margin:"0 auto",padding:"44px 20px"}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(210px,1fr))",gap:18}}>
            {[
              {icon:"🛡️",title:"Safety First",desc:"Strict safety protocols on every trip."},
              {icon:"⚡",title:"Fast Booking",desc:"Book in under 2 minutes, ticket on WhatsApp."},
              {icon:"💺",title:"Comfortable Rides",desc:"Modern, clean vehicles for every journey."},
              {icon:"💳",title:"Mobile Money",desc:"Pay via MTN MoMo or Airtel Money instantly.",logos:true}
            ].map(f=>(
              <Card key={f.title} style={{textAlign:"center"}}>
                ...
                {f.logos&&(
                  <div style={{display:"flex",gap:8,justifyContent:"center",marginTop:10}}>
                    <MTNLogo size={28}/><AirtelLogo size={28}/>
                  </div>
                )}
              </Card>
            ))}
      ...
      <footer style={{background:C.navyMid,borderTop:`1px solid ${C.navyBorder}`,padding:"36px 20px 18px"}}>
        <div style={{maxWidth:1200,margin:"0 auto"}}>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(170px,1fr))",gap:28,marginBottom:28}}>
            ...
            {[
              {title:"Services",links:["Book a Seat","Plan Journey","Hire a Van","Courier/Parcel"]},
              {title:null,links:["Safety Guidelines","FAQ","Contact Us","Lost & Found"]},
              {title:"Contact",links:["+256 700 000000","info@raylane.ug","Nakasero, Kampala","Mon–Sun 5AM–10PM"]}
            ].map((col,ci)=>(
              <div key={ci}>
                {ci===1
                  ? <div className="ral" style={{fontWeight:800,fontSize:11,letterSpacing:".5px",color:C.amber,marginBottom:12,cursor:"default"}}>
                      Support<span
                        title=""
                        onClick={()=>{setLoginRole("admin");setPage("login");}}
                        style={{cursor:"default",userSelect:"none",color:C.amber,fontSize:11}}
                      >.</span>
                    </div>
          ...
          <div style={{borderTop:`1px solid ${C.navyBorder}`,paddingTop:16,display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:6,fontSize:11,color:C.textMuted}}>
            <span>© 2026 Raylane Express Ltd. All rights reserved.</span>
            <div style={{display:"flex",alignItems:"center",gap:12}}>
              <span>Privacy · Terms · Refunds</span>
              <span
                onClick={()=>{setLoginRole("agent");setPage("login");}}
                title="Staff Login"
                style={{cursor:"pointer",fontSize:13,opacity:0.35,transition:"opacity .2s",userSelect:"none"}}
...
const BookingPage=({preselectedTrip,store,currentUser})=>{
  ...
  const [showAuth,setShowAuth]=useState(!currentUser); // show auth panel until logged in or skipped

  useEffect(()=>{if(preselectedTrip){setTrip(preselectedTrip);setStep(2);}}, [preselectedTrip]);

  // When customer authenticates, auto-fill their details
  ...
  return(
    <div style={{minHeight:"100vh",paddingTop:80,maxWidth:900,margin:"0 auto",padding:"80px 20px 40px"}}>
      ...
      {/* Customer Auth Panel — shown at top until logged in or skipped */}
      {showAuth&&step<5&&(
        <CustomerAuthPanel store={store} onAuth={handleCustomerAuth} onSkip={()=>setShowAuth(false)}/>
      )}

      {/* Logged-in customer badge */}
      {loggedInCustomer&&!showAuth&&(
        <div style={{background:C.green+"11",border:`1px solid ${C.green}33`,borderRadius:10,padding:"9px 14px",marginBottom:16,fontSize:12,color:C.green,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          ...
          <button onClick={()=>{setLoggedInCustomer(null);setShowAuth(true);}} style={{background:"none",border:"none",color:C.textMuted,fontSize:11,cursor:"pointer",textDecoration:"underline"}}>Switch</button>
        </div>
      )}
...
const ParcelPage=({store})=>{
  ...
  return(
    <div style={{minHeight:"100vh",paddingTop:80,maxWidth:900,margin:"0 auto",padding:"80px 20px 40px"}}>
      ...
      {/* Customer auth */}
      {showAuth&&!submitted&&(
        <CustomerAuthPanel store={st} onAuth={handleCustomerAuth} onSkip={()=>setShowAuth(false)}/>
      )}
      {loggedInCustomer&&!showAuth&&(
          ...
          <div style={{animation:"fadeUp .3s ease"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:18}}>
              ...
              <Card style={{border:`1px solid ${C.amber}33`}}>
                <h3 className="ral" style={{fontWeight:700,marginBottom:6}}>Hero Background Images</h3>
                <p style={{fontSize:12,color:C.textMuted,marginBottom:14,lineHeight:1.6}}>Upload images to display as animated slideshow in the homepage hero section. Each image is shown for 5 seconds with a cinematic zoom-fade transition.</p>
                <div style={{marginBottom:14}}>
                  <label style={{display:"flex",alignItems:"center",gap:10,background:C.amber+"22",border:`1.5px dashed ${C.amber}66`,borderRadius:10,padding:"14px 16px",cursor:"pointer",transition:"all .2s"}}
                    onMouseEnter={e=>{e.currentTarget.style.background=C.amber+"33";}}
                    onMouseLeave={e=>{e.currentTarget.style.background=C.amber+"22";}}>
                    <span style={{fontSize:22}}>🖼️</span>
                    <div>
                      <div style={{fontWeight:700,fontSize:13,color:C.amber}}>Click to upload image</div>
                      <div style={{fontSize:11,color:C.textMuted}}>JPG, PNG, WebP — max 5MB recommended</div>
                    </div>
                    <input type="file" accept="image/*" style={{display:"none"}} onChange={e=>{
                      const file=e.target.files[0];
                      if(!file) return;
                      const reader=new FileReader();
                      reader.onload=ev=>store.addHeroImage(ev.target.result,file.name);
                ...
                {store.heroImages.length===0?(
                  <div style={{textAlign:"center",padding:"18px 0",color:C.textMuted,fontSize:12,border:`1px dashed ${C.navyBorder}`,borderRadius:10}}>
                    No images uploaded yet. The hero will use the default animated gradient background.
                  </div>
                ):(
                  <div style={{display:"flex",flexDirection:"column",gap:8}}>
                    {store.heroImages.map((img,i)=>(
                      <div key={img.id} style={{display:"flex",alignItems:"center",gap:10,background:C.navyMid,borderRadius:10,padding:"8px 12px",border:`1px solid ${C.navyBorder}`}}>
                        <img src={img.url} alt="" style={{width:52,height:36,objectFit:"cover",borderRadius:6,border:`1px solid ${C.navyBorder}`}}/>
                        <div style={{flex:1,minWidth:0}}>
                          <div style={{fontSize:12,fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{img.name}</div>
                          <div style={{fontSize:10,color:C.textMuted}}>Slide {i+1} · 5s display</div>
                        </div>
                    ...
                    <div style={{fontSize:11,color:C.green,marginTop:4}}>✓ {store.heroImages.length} image{store.heroImages.length>1?"s":""} — homepage hero slideshow active</div>
                  </div>
                )}
              </Card>
