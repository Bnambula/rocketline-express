    ...
    <section aria-label="Our Presence in Uganda" style={{padding:"clamp(56px,7vw,88px) clamp(16px,4vw,40px)",background:"#f8faff"}}>
      <div style={{maxWidth:1280,margin:"0 auto"}}>
        ...
        <div style={{display:"flex",gap:32,alignItems:"flex-start",flexWrap:"wrap",justifyContent:"center"}}>
          ...
          <div ref={mapRef} style={{position:"relative",width:"min(460px,100%)",flexShrink:0,borderRadius:20,overflow:"hidden",boxShadow:"0 12px 48px rgba(11,30,75,0.18)"}}>
              ...
              {UG_CITIES.map(c=>{
                ...
                return(
                  <g key={c.name}
                    ...
                    {/* Drop shadow */}
                    <ellipse cx={c.x} cy={c.y+0.4} rx="1.4" ry="0.55" fill="rgba(0,0,0,0.18)"/>
                    {/* Pin teardrop */}
                    <path d={`M${c.x} ${c.y-5.5} a3.2,3.2 0 1,1 0.001,0 Z`}
                      fill={col} stroke="#fff" strokeWidth="0.6"/>
                    <path d={`M${c.x-1.1} ${c.y-3.4} L${c.x} ${c.y} L${c.x+1.1} ${c.y-3.4}`}
                      fill={col} stroke="#fff" strokeWidth="0.3"/>
                    {/* Inner dot */}
                    <circle cx={c.x} cy={c.y-4.8} r="1.1" fill="#fff" opacity="0.9"/>
                    {/* Label */}
                    <text
                ...
                <div key={c.name}
                  ...
                  <div style={{width:34,height:34,borderRadius:"50%",background:c.main?C.amber+"22":C.blue+"18",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                    <svg width="16" height="20" viewBox="0 0 24 30">
                      <path d="M12 2C7.13 2 3 6.13 3 11c0 7 9 17 9 17s9-10 9-17c0-4.87-4.13-9-9-9zm0 12a3 3 0 100-6 3 3 0 000 6z" fill={c.main?C.amber:C.blue}/>
                    </svg>
                  </div>
                  <div>
