...
const POPULAR_DESTINATIONS=[
  {city:"Mbale",  desc:"Gateway to Mount Elgon",          emoji:"⛰️",  price:30000},
  {city:"Gulu",   desc:"Heart of Northern Uganda",         emoji:"🌿",  price:35000},
  {city:"Fort Portal",desc:"Pearl of Western Uganda",     emoji:"🏔️", price:40000},
  {city:"Jinja",  desc:"Source of the Nile",              emoji:"🌊",  price:15000},
  {city:"Mbarara",desc:"Ankole's vibrant capital",        emoji:"🌄",  price:25000},
];
        ...
        <div style={{display:"flex",gap:2,alignItems:"center"}}>
          ...
          {currentUser?(
            <div style={{display:"flex",alignItems:"center",gap:8}}>
              ...
              <Btn onClick={()=>go("logout")} variant="navy" style={{padding:"6px 14px",fontSize:11,border:"1px solid rgba(255,255,255,0.15)"}}>Logout</Btn>
            </div>
          ):(
            <Btn onClick={()=>go("register")} variant="navy" style={{padding:"7px 16px",fontSize:12,border:"1px solid rgba(255,255,255,0.2)"}}>Login / Register</Btn>
          )}
        </div>
      </div>
    </nav>
...
const HomePage=({setPage,setPreselectedTrip,store,setLoginRole=()=>{}})=>{
  ...
  return(
    <div style={{minHeight:"100vh",paddingTop:64,background:"#F4F6FA"}}>
        ...
        <div style={{maxWidth:1280,margin:"0 auto",position:"relative",width:"100%",padding:"clamp(72px,10vw,120px) clamp(20px,5vw,48px) clamp(96px,12vw,140px)",zIndex:5}}>
          <div style={{display:"grid",gridTemplateColumns:"1fr minmax(0,320px)",gap:"clamp(24px,4vw,56px)",alignItems:"center"}}>
            ...
            <div style={{animation:"fadeUp .5s ease both"}}>
              ...
              <div role="list" style={{display:"flex",gap:20,fontSize:12,color:"rgba(168,186,218,0.7)",flexWrap:"wrap",alignItems:"center"}}>
                <span role="listitem" style={{display:"flex",alignItems:"center",gap:6}}><MTNLogo size={20}/> MTN MoMo</span>
                <span role="listitem" style={{display:"flex",alignItems:"center",gap:6}}><AirtelLogo size={20}/> Airtel Money</span>
                <span role="listitem">💬 WhatsApp receipt</span>
                <span role="listitem">📦 Courier service</span>
              </div>
            </div>
      ...
      {/* WHY RAYLANE */}
      <section aria-label="Why choose Raylane" style={{background:`linear-gradient(135deg,${C.amber} 0%,#F0920A 100%)`,padding:"clamp(56px,7vw,88px) clamp(16px,4vw,40px)"}}>
        <div style={{maxWidth:1280,margin:"0 auto"}}>
          <div style={{textAlign:"center",marginBottom:"clamp(32px,4vw,52px)"}}>
            <p style={{fontFamily:"'Inter',sans-serif",fontWeight:700,fontSize:11,letterSpacing:"0.1em",textTransform:"uppercase",color:"rgba(13,27,62,0.55)",marginBottom:10}}>Our Commitment</p>
            <h2 style={{fontFamily:"'Source Serif 4',Georgia,serif",fontWeight:700,fontSize:"clamp(26px,4vw,42px)",color:C.navy,lineHeight:1.1,marginBottom:14}}>Why Choose Raylane Express?</h2>
            <p style={{fontFamily:"'Inter',sans-serif",color:"rgba(13,27,62,0.65)",fontSize:"clamp(13px,1.5vw,15px)",maxWidth:480,margin:"0 auto",lineHeight:1.75}}>Uganda's most trusted intercity transport — built on safety, speed, and service.</p>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(220px,1fr))",gap:"clamp(14px,2vw,20px)",marginBottom:"clamp(36px,5vw,52px)"}}>
            {[
              {icon:"🛡️",title:"Safety First",desc:"Strict protocols, vetted drivers and insured vehicles on every trip."},
              {icon:"⚡",title:"Book in 2 Minutes",desc:"Instant confirmation. Your ticket arrives on WhatsApp the moment you pay."},
              {icon:"💺",title:"Comfortable Rides",desc:"Clean, modern vehicles for business or leisure — air-conditioned where available."},
              {icon:"💳",title:"Mobile Money",desc:"Pay via MTN MoMo or Airtel Money. No cash, no queues.",logos:true}
            ].map((f,i)=>(
              <div key={f.title} style={{background:"rgba(255,255,255,0.96)",borderRadius:20,padding:"clamp(20px,2.5vw,28px)",textAlign:"center",boxShadow:"0 6px 24px rgba(13,27,62,0.12)",transition:"transform .25s,box-shadow .25s"}}
                onMouseEnter={e=>{e.currentTarget.style.transform="translateY(-4px)";e.currentTarget.style.boxShadow="0 16px 44px rgba(13,27,62,0.18)";}}
                onMouseLeave={e=>{e.currentTarget.style.transform="";e.currentTarget.style.boxShadow="0 6px 24px rgba(13,27,62,0.12)";}}>
                <div style={{width:56,height:56,borderRadius:"50%",background:C.amber+"20",margin:"0 auto 16px",display:"flex",alignItems:"center",justifyContent:"center",fontSize:26}} aria-hidden="true">{f.icon}</div>
                <h3 style={{fontFamily:"'Source Serif 4',Georgia,serif",fontWeight:700,fontSize:17,marginBottom:8,color:C.navy}}>{f.title}</h3>
                <p style={{fontFamily:"'Inter',sans-serif",fontSize:13,color:"#5B7299",lineHeight:1.75}}>{f.desc}</p>
                {f.logos&&<div style={{display:"flex",gap:8,justifyContent:"center",marginTop:12}}><MTNLogo size={28}/><AirtelLogo size={28}/></div>}
              </div>
            ))}
          </div>
          <div style={{textAlign:"center"}}>
            <button onClick={()=>setPage("book")} style={{background:C.navy,color:"#fff",border:"none",borderRadius:14,padding:"clamp(13px,1.5vw,16px) clamp(32px,4vw,48px)",fontFamily:"'Inter',sans-serif",fontWeight:700,fontSize:"clamp(13px,1.5vw,15px)",cursor:"pointer",boxShadow:"0 8px 28px rgba(13,27,62,0.35)",transition:"all .25s"}} onMouseEnter={e=>{e.currentTarget.style.transform="translateY(-2px)";e.currentTarget.style.boxShadow="0 14px 40px rgba(13,27,62,0.45)";}} onMouseLeave={e=>{e.currentTarget.style.transform="";e.currentTarget.style.boxShadow="0 8px 28px rgba(13,27,62,0.35)";}}>Reserve Your Seat Today</button>
          </div>
        </div>
      </section>
      ...
      <section aria-label="Destinations and routes" style={{background:"#F4F6FA",padding:"clamp(56px,7vw,96px) clamp(16px,4vw,40px)"}}>
        <div style={{maxWidth:1280,margin:"0 auto"}}>
          <div style={{textAlign:"center",marginBottom:"clamp(32px,4vw,52px)"}}>
            <p style={{fontFamily:"'Inter',sans-serif",fontWeight:700,fontSize:11,letterSpacing:"0.1em",textTransform:"uppercase",color:C.amber,marginBottom:10}}>Where We Go</p>
            <h2 style={{fontFamily:"'Source Serif 4',Georgia,serif",fontWeight:700,fontSize:"clamp(26px,4vw,42px)",color:C.navy,lineHeight:1.1,marginBottom:14}}>Where Do You Want to Go?</h2>
            <p style={{fontFamily:"'Inter',sans-serif",color:"#5B7299",fontSize:"clamp(13px,1.5vw,15px)",maxWidth:500,margin:"0 auto",lineHeight:1.75}}>Daily departures from multiple cities. First come, first served.</p>
            <div style={{width:48,height:3,background:`linear-gradient(90deg,${C.amber},#FFB940)`,borderRadius:2,margin:"16px auto 0"}}/>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(clamp(160px,18vw,220px),1fr))",gap:"clamp(12px,2vw,20px)",marginBottom:"clamp(28px,4vw,48px)"}}>
            {POPULAR_DESTINATIONS.map((d,i)=>(
              <button key={d.city} onClick={()=>setPage("book")} aria-label={`Book to ${d.city} from ${formatUGX(d.price)}`}
                style={{background:"#FFFFFF",border:"1px solid #E4EAF5",borderRadius:20,padding:"clamp(18px,2.5vw,28px) clamp(14px,2vw,20px)",cursor:"pointer",textAlign:"left",width:"100%",transition:"all .25s",fontFamily:"inherit"}}
                onMouseEnter={e=>{e.currentTarget.style.transform="translateY(-4px)";e.currentTarget.style.boxShadow="0 14px 40px rgba(13,27,62,0.11)";e.currentTarget.style.borderColor=C.amber+"55";}}
                onMouseLeave={e=>{e.currentTarget.style.transform="";e.currentTarget.style.boxShadow="";e.currentTarget.style.borderColor="#E4EAF5";}}>
                <div style={{fontSize:28,marginBottom:12}} aria-hidden="true">{d.emoji}</div>
                <div style={{fontFamily:"'Source Serif 4',Georgia,serif",fontWeight:700,fontSize:20,color:C.navy,marginBottom:5}}>{d.city}</div>
                <div style={{fontFamily:"'Inter',sans-serif",fontSize:12,color:"#6B82A8",marginBottom:16,lineHeight:1.5}}>{d.desc}</div>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <span style={{fontFamily:"'Source Serif 4',Georgia,serif",fontWeight:700,fontSize:15,color:C.amber}}>from {formatUGX(d.price)}</span>
                  <div style={{width:28,height:28,background:C.navy,borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,color:"#fff"}} aria-hidden="true">→</div>
                </div>
              </button>
            ))}
          </div>
