...
export default function App() {
  ...
  async function addAgent() {
    ...
    try {
      const rows = await sb.insert("users", {
        ...
        password_hash: `PLAIN:${newAgent.pin}`,
        role:          "agent",
        is_active:     true,
      });
      const a = rows[0];
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f4f6fa", color: "#1a2540", fontFamily: "'Nunito Sans', 'Segoe UI', sans-serif" }}>
      ...
      <div style={{borderTop:"1px solid #dde3ef",padding:"20px 16px",background:"#060F20"}}>
        <div style={{maxWidth:780,margin:"0 auto",display:"flex",gap:10,justifyContent:"center",flexWrap:"wrap"}}>
          <button onClick={()=>{setStep("admin");setAdminStep("locked");}} style={{display:"flex",alignItems:"center",gap:8,background:"rgba(229,38,26,0.08)",border:"1px solid rgba(229,38,26,0.3)",borderRadius:10,color:"#FF6B5B",fontSize:13,padding:"12px 22px",cursor:"pointer",fontWeight:700}}>
            🔒 Admin / Staff Login
          </button>
          <button onClick={()=>setStep("feedback")} style={{display:"flex",alignItems:"center",gap:8,background:"rgba(245,166,35,0.08)",border:"1px solid rgba(245,166,35,0.3)",borderRadius:10,color:"#F5A623",fontSize:13,padding:"12px 22px",cursor:"pointer",fontWeight:700}}>
            ⭐ Leave Feedback
          </button>
        </div>
      </div>
      <div style={{borderTop:"1px solid #dde3ef",padding:"36px 16px",textAlign:"center",background:"#1a2540",color:"#fff"}}>
        <div style={{maxWidth:780,margin:"0 auto"}}>
          <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:10,marginBottom:14}}>
            <div style={{display:"flex",alignItems:"center",gap:16}}>
              <img src={LOGO_R} alt="R" style={{height:56,width:"auto",objectFit:"contain",opacity:0.85}} />
              <div style={{textAlign:"left"}}>
                <img src={LOGO_MAIN} alt="Rocketline Express" style={{height:52,width:"auto",objectFit:"contain"}} />
                <div style={{fontSize:9,color:"rgba(255,255,255,0.35)",letterSpacing:"5px",marginTop:3,textTransform:"uppercase"}}>ISHENDAAA · Premier Travel, Linking Cities</div>
              </div>
            </div>
          </div>
          <div style={{display:"flex",justifyContent:"center",gap:12,flexWrap:"wrap",marginBottom:16}}>
            <a href="tel:+256701196725" style={{display:"inline-flex",alignItems:"center",gap:6,color:"rgba(255,255,255,0.6)",fontSize:13,textDecoration:"none"}}>📞 +256 701 196 725</a>
            <a href="https://wa.me/256701196725" target="_blank" rel="noreferrer" style={{display:"inline-flex",alignItems:"center",gap:6,color:"#25D366",fontSize:13,textDecoration:"none"}}>💬 WhatsApp Customer Care</a>
          </div>
          <div style={{fontSize:12,color:"rgba(255,255,255,0.4)",marginBottom:6}}>
            Payments secured by MTN MoMo &amp; Airtel Money · Free water on every trip
          </div>
        </div>
      </div>
    </div>
