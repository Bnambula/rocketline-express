      ...
      <div style={{maxWidth:1280,margin:"0 auto",padding:"0 20px",height:64,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        ...
        <div className="hide-mobile" style={{display:"flex",gap:2,alignItems:"center"}}>
          ...
          {currentUser?(
            ...
            <Btn onClick={()=>setPage("customer-login")} variant="navy" style={{padding:"7px 16px",fontSize:12,marginLeft:6,background:C.navyLight,color:C.textSecondary,border:`1px solid ${C.navyBorder}`}}>👤 My Account</Btn>
          )}
        </div>
        {/* Mobile hamburger */}
      ...
      <footer style={{background:C.navy,borderTop:`1px solid rgba(255,255,255,0.1)`,padding:"44px 20px 20px"}}>
        <div style={{maxWidth:1200,margin:"0 auto"}}>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(170px,1fr))",gap:32,marginBottom:36}}>
            <div>
              <div className="playfair" style={{fontWeight:800,fontSize:18,color:"#ffffff",marginBottom:12}}>Raylane</div>
              <p style={{fontSize:13,color:"rgba(255,255,255,0.5)",lineHeight:1.8}}>Safe, reliable transport across Uganda since 2018.</p>
            </div>
            {[
              {title:"Services",links:[{label:"Book a Seat",fn:()=>setPage("book")},{label:"Plan Journey",fn:()=>setPage("plan")},{label:"Hire a Van",fn:()=>setPage("plan")},{label:"Courier / Parcel",fn:()=>setPage("parcel")}]},
              {title:"Support·",links:[{label:"Safety Guidelines",fn:()=>setPage("safety")},{label:"FAQ",fn:()=>setPage("faq")},{label:"Contact Us",fn:null},{label:"Lost & Found",fn:null}]},
              {title:"Contact",links:[{label:"+256 700 000000",fn:null},{label:"info@raylane.ug",fn:null},{label:"Nakasero, Kampala",fn:null},{label:"Mon–Sun 5AM–10PM",fn:null}]}
            ].map(col=>(
              <div key={col.title}>
                {/* "Support·" — the fullstop is the secret staff login trigger */}
                <div className="ral" style={{fontWeight:800,fontSize:11,letterSpacing:".5px",color:C.amberLight,marginBottom:14,textTransform:"uppercase",display:"flex",alignItems:"center",gap:0}}>
                  {col.title==="Support·"?(
                    <>
                      <span>Support</span>
                      <span onClick={()=>onStaffLogin&&onStaffLogin()} title="" style={{cursor:"default",color:"rgba(255,255,255,0.02)",fontSize:8,lineHeight:1,userSelect:"none",padding:"0 1px",opacity:0}} aria-hidden="true">.</span>
                    </>
                  ):col.title}
                </div>
                {col.links.map((l,li)=>(
                  <div key={l.label} onClick={l.fn||undefined} style={{fontSize:13,color:"rgba(255,255,255,0.5)",marginBottom:8,cursor:l.fn?"pointer":"default",transition:"color .15s"}}
                    onMouseEnter={e=>{if(l.fn)e.currentTarget.style.color=C.amberLight;}}
...
const AdminDashboard=({store,currentUser,onLogout})=>{
  ...
  const sidebar=[
    ...
    {id:"finance-full",icon:"📊",label:"Finance"},
    {id:"finance-revenue",icon:"💰",label:"↳ Revenue"},
    {id:"finance-expenses",icon:"💸",label:"↳ Cost Management"},
  ...
  return(
    <div style={{display:"flex",minHeight:"100vh",paddingTop:64,background:C.navy}}>
      ...
      <div style={{marginLeft:210,flex:1,padding:"24px"}}>
        ...
        {/* ── COST MANAGEMENT ── */}
        {section==="finance-expenses"&&<AdminCostManagement store={store}/>}
