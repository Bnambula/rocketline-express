          ...
          <div>
            ...
            {[['📍','Real-time tracking','Know exactly where your parcel is at all times'],['🛡️','Insured delivery','3% optional insurance for high-value items'],['📲','SMS notifications','Sender & recipient get live updates'],['⚡','Same-day dispatch','Parcels leave with the next available bus']].map(([ic,t,d])=>(
              <div key={t} style={{ display:'flex', gap:12, alignItems:'flex-start', marginBottom:14 }}>
                ...
                <div>
                  <div style={{ fontFamily:'var(--font-head)', fontWeight:700, fontSize:14 }}>{t}</div>
                  <div style={{ color:'var(--gray-text)', fontSize:13 }}>{d}</div>
                </div>
              </div>
            ))}
            <button onClick={() => navigate('/parcels')} style={{ marginTop:10, padding:'13px 28px', borderRadius:20, background:'var(--blue)', color:'var(--white)', border:'none', fontFamily:'var(--font-head)', fontWeight:800, fontSize:14, cursor:'pointer', boxShadow:'var(--shadow-md)' }}>
              📦 Send a Parcel Now
            </button>
          </div>
          <div>
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14 }}>
              {parcels.map(p => (
                <div key={p.id} onClick={() => setSelected(selected===p.id?null:p.id)}
                  ...
                  <div style={{ fontFamily:'var(--font-head)', fontWeight:800, fontSize:13, marginBottom:5 }}>{p.name}</div>
                  <div style={{ fontSize:11, opacity:.75, marginBottom:10 }}>{p.desc}</div>
                  <div style={{ fontFamily:'var(--font-head)', fontWeight:900, fontSize:15, color:selected===p.id?'var(--gold)':'var(--blue)' }}>UGX {p.price.toLocaleString()}</div>
                  <div style={{ fontSize:10, opacity:.7, marginTop:2 }}>Max {p.maxWeight}</div>
                </div>
              ))}
            </div>
            {selected && (
              <button onClick={() => navigate(`/parcels?type=${selected}`)} style={{ width:'100%', marginTop:14, padding:'13px', borderRadius:14, background:'var(--gold)', color:'var(--blue)', border:'none', fontFamily:'var(--font-head)', fontWeight:800, fontSize:14, cursor:'pointer', boxShadow:'0 4px 16px rgba(255,199,44,0.4)' }}>
