...
import { BusSeat55, BusSeat65, TaxiSeat14, SeatLegend } from '../ui/SharedComponents'
...
export default function SeatSelectionSection() {
  ...
  return (
    <section style={{ background:'var(--gray-light)', padding:'72px 0' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'260px 1fr', gap:20, alignItems:'start' }}>
          ...
          <div>
            <div style={{ background:'var(--blue)', borderRadius:16, padding:18, color:'var(--white)', marginBottom:12 }}>
              ...
              <div style={{ fontFamily:'var(--font-head)', fontWeight:800, fontSize:16, marginBottom:3 }}>{vehicle.label} Coach</div>
            ...
            {selected.length > 0 ? (
              <div style={{ background:'var(--white)', borderRadius:16, padding:18, boxShadow:'var(--shadow-md)', border:'2px solid var(--gold)' }}>
                <div style={{ fontSize:10, fontWeight:700, color:'var(--gray-text)', fontFamily:'var(--font-head)', textTransform:'uppercase', letterSpacing:1, marginBottom:8 }}>Selected</div>
                <div style={{ display:'flex', flexWrap:'wrap', gap:5, marginBottom:12 }}>
                  {selected.sort((a,b)=>a-b).map(s => (
                    <span key={s} style={{ background:'var(--gold)', color:'var(--blue)', padding:'3px 10px', borderRadius:7, fontFamily:'var(--font-head)', fontWeight:800, fontSize:12 }}>{s}</span>
                  ))}
                </div>
                ...
                <button onClick={() => navigate(`/book?seats=${selected.join(',')}&vehicle=${vType}`)}
                  style={{ width:'100%', padding:'12px', borderRadius:12, background:'var(--gold)', color:'var(--blue)', border:'none', fontFamily:'var(--font-head)', fontWeight:800, fontSize:14, cursor:'pointer' }}>
                  Continue →
                </button>
              </div>
            ) : (
              <div style={{ background:'var(--white)', borderRadius:14, padding:18, textAlign:'center', color:'var(--gray-text)', boxShadow:'var(--shadow-sm)' }}>
                ...
                <div style={{ fontFamily:'var(--font-head)', fontWeight:700, fontSize:13 }}>Tap a seat to select</div>
          ...
          <div>
            ...
            <div style={{ marginTop:10, display:'flex', gap:14, justifyContent:'center', flexWrap:'wrap' }}>
              {[['Total',vehicle.seats,'#1d4ed8'],['Booked',vehicle.booked.length,'#dc2626'],['Available',vehicle.seats-vehicle.booked.length,'#15803d'],['Selected',selected.length,'#92400e']].map(([l,v,c])=>(
                <div key={l} style={{ fontFamily:'var(--font-head)', fontWeight:700, fontSize:12, color:'var(--gray-text)' }}>
      ...
      <style>{`@media(max-width:900px){.seat-layout{grid-template-columns:1fr!important;}}`}</style>
    </section>
  )
}
