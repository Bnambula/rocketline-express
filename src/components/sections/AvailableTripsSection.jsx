      ...
      <div className="container">
        <div style={{ display:'flex', alignItems:'flex-end', justifyContent:'space-between', marginBottom:24, flexWrap:'wrap', gap:14 }}>
          ...
          <div style={{ display:'flex', gap:7, alignItems:'center', flexWrap:'wrap' }}>
            ...
            {['price','rating','time'].map(s=>(
              <button key={s} onClick={()=>setSortBy(s)} style={{ padding:'6px 14px', borderRadius:20, border:`1.5px solid ${sortBy===s?'var(--blue)':'var(--gray-mid)'}`, background:sortBy===s?'var(--blue)':'var(--white)', color:sortBy===s?'var(--white)':'var(--dark)', fontFamily:'var(--font-head)', fontWeight:700, fontSize:12, cursor:'pointer', textTransform:'capitalize', transition:'all .18s' }}>{s}</button>
            ))}
          </div>
        </div>
        ...
        {alert && liveTrips.some(t=>t.seats_total-t.seats_booked<6) && (
          <div style={{ background:'linear-gradient(135deg,#fff3cd,#ffeaa7)', border:'1px solid #f59e0b', borderRadius:12, padding:'13px 18px', marginBottom:18, display:'flex', alignItems:'center', justifyContent:'space-between', gap:12 }}>
            <div style={{ display:'flex', alignItems:'center', gap:9 }}>
              ...
              <span style={{ fontFamily:'var(--font-head)', fontWeight:700, fontSize:13, color:'#92400e' }}>High demand — some routes have fewer than 6 seats remaining!</span>
            </div>
        ...
        {liveTrips.length === 0 ? (
          <div style={{ textAlign:'center', padding:'40px', background:'var(--white)', borderRadius:16 }}>
            ...
            <h3 style={{ fontFamily:'var(--font-head)', fontWeight:800, fontSize:18 }}>No approved trips right now</h3>
            ...
            {sorted.map(trip=>(
              <div key={trip.id} style={{ background:'var(--white)', borderRadius:16, padding:'16px 18px', boxShadow:'var(--shadow-sm)', transition:'all .2s', opacity:trip.full?.7:1 }}
                ...
                <div style={{ display:'grid', gridTemplateColumns:'auto 1fr auto', gap:14, alignItems:'center' }}>
                  <div style={{ width:48, height:48, borderRadius:13, background:'rgba(11,61,145,0.08)', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:'var(--font-head)', fontWeight:900, fontSize:15, color:'var(--blue)', flexShrink:0 }}>
                    {trip.op?.company_name?.[0]||'?'}{trip.op?.company_name?.split(' ')[1]?.[0]||''}
                  </div>
                  <div>
                    <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:4, flexWrap:'wrap' }}>
                      <span style={{ fontFamily:'var(--font-head)', fontWeight:800, fontSize:15 }}>{trip.op?.company_name||'Unknown'}</span>
                    ...
                    <div style={{ display:'flex', alignItems:'center', gap:12, flexWrap:'wrap' }}>
                      {trip.op?.rating&&(
                        <div style={{ display:'flex', alignItems:'center', gap:3 }}>
                          ...
                          <span style={{ fontFamily:'var(--font-head)', fontWeight:700, fontSize:12 }}>{trip.op.rating}</span>
                  ...
                  <div style={{ textAlign:'right', display:'flex', flexDirection:'column', alignItems:'flex-end', gap:8 }}>
                    ...
                    {trip.full
                      ...
                      : <button onClick={()=>navigate(`/book?from=${trip.from}&to=${trip.to}&trip=${trip.id}`)} style={{ padding:'9px 18px', borderRadius:20, background:'var(--gold)', color:'var(--blue)', border:'none', fontFamily:'var(--font-head)', fontWeight:800, fontSize:13, cursor:'pointer', boxShadow:'0 2px 10px rgba(255,199,44,0.35)', transition:'all .18s' }}
